...
from GUI.ms import Ui_PeakFormer
from main import *
...
class MyWindow(QMainWindow):
    ...
    def on_eic_build_clicked(self):
        if self.args.source and self.args.feature:
            ...
            self.ui.textBrowser.append(f"{self.current_time}  Start to build EIC, wait for a moment please")
...
class EicPredictThread(QtCore.QThread):
    ...
    def run(self):
        if self.with_plot:
            ...
            self.predict_finished.emit("Successfully predict EIC with plot", results)
        else:
            results = build_predictor(self.model_path, self.images_path, plot=False)
            self.predict_finished.emit("Successfully predict EIC without plot", results)
...
class EicQuantifyThread(QtCore.QThread):
    ...
    def run(self):
        ...
        self.quantify_finished.emit("Successfully quantify EIC", area)
